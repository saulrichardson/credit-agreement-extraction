...
Assumes file layout:
  runs/<run>/llm_qa/pricing_second_pass_dg_nano_v2_<item>.txt  (or shared v2 file)
  runs/<run>/retrieval/<item>_snippets_highsimple.jsonl
  runs/<run>/normalized/<item>/canonical.txt
  runs/<run>/retrieval/<item>_snippets_covenant.jsonl
...
def resolve_pricing_json(run_id: str, item: str) -> Path:
    base = Path("runs")
    base /= run_id / "llm_qa"
...
def main() -> None:
    ...
    for item in items:
        ...
        pricing_json = resolve_pricing_json(run_id, item)
        base = Path("runs")
        base /= run_id
        ...
        print(f"   Metric defs -> runs/{run_id}/llm_qa/metric_definitions/{item}_metric_definitions_clean.json")

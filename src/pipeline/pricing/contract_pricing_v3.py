...
from pipeline.pricing.contract_pricing import (
...
from pipeline.pricing.contract_pricing_planner import plan_contract_pricing_v2
from pipeline.pricing.contract_pricing_planner_table_pack import plan_contract_pricing_table_pack
from pipeline.pricing.contract_schemas import (
...
from pipeline.pricing.doc_ir import build_doc_ir, write_doc_ir
from pipeline.pricing.doc_tools_full import DocBrowserFull
from pipeline.pricing.table_critic_schemas import TableCritique
from pipeline.pricing.semantic_pricing_scan import semantic_pricing_scan
...
def run_contract_pricing_v3(
    ...
    *,
    ...
    async def _process(item_id: str, client: Any) -> None:
        async with sem:
            try:
                # Clear stale status files when rerunning a subset.
                for suffix in (".error.txt", ".warning.txt"):
                    stale = out_root / f"{item_id}{suffix}"
                    if stale.exists():
                        stale.unlink()
                ...
                for table_plan in plan_model.selected_tables:
                    ...
                    for attempt in range(1, max(1, attempts) + 1):
                        ...
                        try:
                            ...
                            (per_table_dir / f"{table_anchor_id}.attempt{attempt}.raw.txt").write_text(raw or "")
                        except Exception as exc:
                    ...
                    if table_anchor_id not in extracts_by_table:
                        table_failures.append(
                            f"{table_anchor_id}: failed after {used} attempts (last_error={last_err})"
                        )
                        continue

                # If all planned tables failed, emit a valid empty artifact and warn (do not fail the whole item).
                if not extracts_by_table:
                    artifact = ContractPricingArtifact(
                        ...
                        pricing=ContractPricingModel(issuer=None, agreement={}, rate_options=[], pricing_regimes=[]),
                    ...
                    (out_root / f"{item_id}.warning.txt").write_text(
                        "All planned tables failed during extraction.\n"
                        + ("\n".join(table_failures) if table_failures else "")
                        + "\n"
                    )
                    return
                ...
                successful_table_contexts = [
                    tc for tc in table_contexts if str((tc.get("table") or {}).get("table_anchor_id") or "") in extracts_by_table
                ]
                regimes = _build_regimes_from_tables(
                    table_contexts=successful_table_contexts, extracts_by_table=extracts_by_table
                )
                ...
                context = {
                    "item_id": item_id,
                    "tables": [tc["table"] for tc in successful_table_contexts],
                ...
                if table_failures:
                    (out_root / f"{item_id}.warning.txt").write_text(
                        "One or more planned tables failed and were excluded from final pricing output.\n"
                        + "\n".join(table_failures)
                        + "\n"
                    )

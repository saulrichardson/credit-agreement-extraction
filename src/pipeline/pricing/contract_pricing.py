...
from pipeline.pricing.pricing_index import build_pricing_index, score_pricing_table
from pipeline.pricing.pricing_heuristics import (
...
def run_contract_pricing(
    ...
    *,
    ...
    async def _process(item_id: str, client: Any) -> None:
        async with sem:
            try:
                # Clear stale per-item status files from prior reruns.
                ...
                if not (context.get("tables") or []):
                    empty_artifact = ContractPricingArtifact(
                    ...
                    (out_root / f"{item_id}.json").write_text(json.dumps(empty_artifact.model_dump(), indent=2))

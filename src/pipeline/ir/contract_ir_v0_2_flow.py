...
from pipeline.core.config import REQUIRED_MODEL, REQUIRED_REASONING, Paths
...
from pipeline.evidence.excerpt_packs import ExcerptPackError, build_excerpt_pack_from_canonical, expand_anchor_ids, order_anchor_ids
from pipeline.evidence.indexing_v2 import run_indexing_v2
from pipeline.llm.gateway import DEFAULT_GATEWAY_URL, _ensure_gateway_client_sync
from pipeline.evidence.retrieval_v2 import render_snippets_v2
from pipeline.schemas_v2 import IndexingSelectionV2Artifact
from pipeline.utils import assert_exists, load_manifest, manifest_items, prompt_view_path
...
def prepare_run_inputs_from_source(*, dest_paths: Paths, source_paths: Paths) -> None:
    """Clone run inputs into a new run for cleanliness.

    In addition to normalized + manifest, opportunistically clone indexing/retrieval artifacts
    so downstream flows can skip expensive recomputation when source artifacts already exist.
    """
    ...
    source_indexing_v2 = source_paths.run_dir / "indexing_v2"
    if source_indexing_v2.exists():
        _copytree_clean(source_indexing_v2, dest_paths.run_dir / "indexing_v2")
    source_retrieval_v2 = source_paths.run_dir / "retrieval_v2"
    if source_retrieval_v2.exists():
        _copytree_clean(source_retrieval_v2, dest_paths.run_dir / "retrieval_v2")
...
def run_contractir_v0_2_flow(
    *,
    ...
    # Auto-skip heavy prerequisite stages when outputs already exist for all selected items.
    if not skip_indexing:
        indexing_dir = paths.run_dir / "indexing_v2"
        if indexing_dir.exists() and all((indexing_dir / f"{item_id}_anchors.json").exists() for item_id in selected_item_ids):
            skip_indexing = True
    if not skip_retrieval:
        retrieval_dir = paths.run_dir / "retrieval_v2"
        if retrieval_dir.exists() and all((retrieval_dir / f"{item_id}_snippets.jsonl").exists() for item_id in selected_item_ids):
            skip_retrieval = True

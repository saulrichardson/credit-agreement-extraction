...
from pipeline.evidence.excerpt_packs import build_excerpt_pack_from_canonical, expand_anchor_ids
from pipeline.llm.gateway import DEFAULT_GATEWAY_URL, _ensure_gateway_client_async
from pipeline.compile.indexing_pricing_definitions_v1_schemas import PricingDefinitionsIndexingSelectionV1
...
def _retry_prompt(
    ...
    *,
    ...
    return (
        ...
        "- Ensure the top-level object has exactly: schema_version, metric_name, definitions, unresolved_dependencies.\n"
...
def _norm_for_substring(s: str) -> str:
    ...
    s2 = s2.replace("\\n", " ").replace("\\r", " ").replace("\\t", " ")
    # Strip common OCR/page-break artifacts like "-4-" that split otherwise verbatim text.
    s2 = re.sub(r"(?<!\w)-\d{1,4}-(?!\w)", " ", s2)
    # Treat orphaned line-break hyphens as whitespace for grounding checks (e.g., "plus - interest expense").
    s2 = re.sub(r"\s-\s+(?=[A-Za-z])", " ", s2)
    ...
    if dv is not None:
        ...
        if dv.strip():
            ...
            if dv_norm2:
                ...
                if dv_norm2 and dv_norm2 not in src_norm:
                    issues.append(
                        "compiler metric.definition_verbatim is not a substring of concatenated source_refs text "
                        "(kept as-is; source_refs may be incomplete or mismatched)"
                    )
...
def _validate_compiler_output_v2_ast(
    *,
    ...
    # Backward-readable input tolerance: if metric_name is omitted, inject it deterministically.
    if "metric_name" not in raw_json:
        raw_json["metric_name"] = metric_name
        issues.append("missing_metric_name_filled")

    allowed_top_keys = {"schema_version", "metric_name", "definitions", "unresolved_dependencies"}
    ...
    if missing_top or extra_top:
        raise RuntimeError(
            "compiler output must have exactly keys {schema_version, metric_name, definitions, unresolved_dependencies} "
    ...
    metric_name_top = raw_json.get("metric_name")
    if not isinstance(metric_name_top, str) or not metric_name_top.strip():
        raise RuntimeError("compiler output metric_name must be a non-empty string")
    if metric_name_top.strip() != metric_name:
        raise RuntimeError(
            f"compiler output metric_name must equal input TARGET_TERM_NAME {metric_name!r}; got {metric_name_top!r}"
        )
    ...
    if dv_norm not in ctx_norm:
        ...
        try:
            ...
            diag = ""
        issues.append(
            "compiler definition.definition_verbatim is not a substring of provided contexts "
            "(after normalization; kept as-is for review)"
            + diag
        )
    ...
    if dv_norm not in _norm_for_substring(src_text).lower():
        issues.append(
            "compiler definition.definition_verbatim is not a substring of concatenated source_refs text "
            "(kept as-is; source_refs may be incomplete or mismatched)"
        )
    ...
    for idx, t in enumerate(input_terms):
        ...
        if _norm_for_substring(t).lower() not in dv_norm:
            issues.append(
                f"compiler definition.input_terms[{idx}] is not a verbatim substring of definition_verbatim "
                "(kept as-is; review for normalization drift or model overreach)"
            )
    ...
    for idx, r in enumerate(clauses):
        ...
        if _norm_for_substring(text).lower() not in dv_norm:
            issues.append(
                f"compiler definition.clauses[{idx}].text is not a verbatim substring of definition_verbatim "
                "(kept as-is; review grounding)"
            )
    ...
    def _validate_ast(node: Any, *, path: str) -> None:
        ...
        if node_type == "term":
            ...
            if _norm_for_substring(term).lower() not in dv_norm:
                issues.append(
                    f"compiler definition.expression_ast{path}.term is not a verbatim substring of definition_verbatim "
                    "(kept as-is; review grounding)"
                )
            return
        if node_type == "number":
            ...
            if raw is not None:
                ...
                if _norm_for_substring(raw).lower() not in dv_norm:
                    issues.append(
                        f"compiler definition.expression_ast{path}.raw is not a verbatim substring of definition_verbatim "
                        "(kept as-is; review grounding)"
                    )
            return
        if node_type == "op":
        ...
        if node_type == "opaque_text":
            ...
            if text.strip() and _norm_for_substring(text).lower() not in dv_norm:
                issues.append(
                    f"compiler definition.expression_ast{path}.text is not a verbatim substring of definition_verbatim "
                    "(kept as-is; review grounding)"
                )
            return
    ...
    async def _process_item(item_id: str, client: Any) -> None:
        ...
        for mt in metric_targets:
            ...
            if isinstance(defs, list) and defs and isinstance(defs[0], dict):
                d0 = dict(defs[0])
                top_metric = doc.get("metric_name")
                if isinstance(top_metric, str) and top_metric.strip() and "metric_name" not in d0:
                    d0["metric_name"] = top_metric.strip()
                compiled.append(d0)
